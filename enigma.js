...
var extend = index.bind(null, true);
var JSONPatch = {};
...
var Session = function () {
  ...
  */
  function Session(options) {
    ...
    session.on('handle:changed', function (handle) {
      return session.apis.onHandleChanged(handle);
    });
    session.on('handle:closed', function (handle) {
      return session.apis.onHandleClosed(handle);
    });
    session.on('closed', function () {
      return session.apis.onSessionClosed();
    });
  }
  ...
  */
  ...
  createClass(Session, [{
    ...
    */
    ...
    key: 'onNotification',
    value: function onNotification(response) {
    ...
    */
    ...
    key: 'onTraffic',
    value: function onTraffic(dir, data) {
    ...
    */
    ...
    value: function getObjectApi(args) {
      ...
      api = this.definition.generate(type).create(this, handle, id, this.protocol.delta, customType);
    ...
    */
    ...
    value: function open() {
      ...
      if (!this.globalPromise) {
        var args = { handle: -1, id: 'Global', type: 'Global', customType: 'Global' };
    ...
    */
    ...
    value: function send(request) {
      ...
      var promise = this.intercept.execute(response, request).then(function (res) {
  ...
  */
  ...
  createClass(Intercept, [{
    ...
    */
    ...
    value: function processMultipleOutParamInterceptor(meta, response) {
    ...
    */
    ...
    value: function processOutInterceptor(meta, response) {
    ...
    */
    ...
    key: 'execute',
    value: function execute(promise, meta) {
      var _this = this;

      return this.interceptors.reduce(function (interception, interceptor) {
        return interception.then(interceptor.onFulfilled && interceptor.onFulfilled.bind(_this, meta), interceptor.onRejected && interceptor.onRejected.bind(_this, meta));
      }, promise);
    }
  }]);
  return Intercept;
...
var ApiCache = function (_KeyValueCache) {
  ...
  createClass(ApiCache, [{
    ...
    * @emits api#changed
    */
    value: function onHandleChanged(handle) {
      var api = this.getApi(handle);
      if (api) {
        api.emit('changed');
      }
    }
    ...
    */
    ...
    key: 'onHandleClosed',
    value: function onHandleClosed(handle) {
      var api = this.getApi(handle);
      if (api) {
        api.emit('closed');
        this.remove(handle);
    ...
    */
    ...
    value: function onSessionClosed() {
      this.getApis().forEach(function (entry) {
